...
# -------------------------------------------------------------
# Location: 20_GUI/02_Configure_Parametric_Simulations/GUI_parametric.R
# -------------------------------------------------------------
# Overall description
...
#
# 14_Optimization_parameters.csv:
...
#
# 12_Control_parameters.csv:
#   control_type (checkbox: "modes", "setpoints"),
...
#   keep_logs      (whether to keep Slurm .err/.out log files)
#   max_requeue    (maximum automatic re-queue attempts for failed configs)
# -------------------------------------------------------------
# Inputs
...
# -------------------------------------------------------------
# Outputs
#   31_SCC_Simulation/Optim_parameters.csv
#   31_SCC_Simulation/scc_settings.sh
#   31_SCC_Simulation/Extraction_code.txt
# -------------------------------------------------------------
# Code outline
#   1. Path resolution
...
#     shiny::runApp("20_GUI/02_Configure_Parametric_Simulations")
...
# The app lives at 20_GUI/02_Configure_Parametric_Simulations/
...
# -------------------------------------------------------------
{
  ...
  if (!grepl("02_Configure_Parametric_Simulations", app_dir, fixed = TRUE)) {
    candidate <- file.path(app_dir, "20_GUI", "02_Configure_Parametric_Simulations")
    if (dir.exists(candidate)) {
      app_dir <- normalizePath(candidate)
    } else {
      stop(
        "GUI_parametric.R: cannot locate '20_GUI/02_Configure_Parametric_Simulations' ",
        "under '", app_dir, "'.  ",
  ...
  scc_path  <- file.path(repo_root, "31_SCC_Simulation")
}
...
ui <- fluidPage(
  ...
  wellPanel(
    h4(icon("sliders"), " Optimization Parameters",
       tags$small(em(" (14_Optimization_parameters.csv)"))),
  ...
  wellPanel(
    h4(icon("cog"), " Control Parameters",
       tags$small(em(" (12_Control_parameters.csv)"))),
    fluidRow(
      column(6,
             checkboxGroupInput(
               "control_type",
               "Control Type",
               choices  = c("Modes"     = "modes",
                            "Setpoints" = "setpoints"),
               selected = trimws(unlist(strsplit(cfg_chr("control_type"), ",")))
             )
      ),
      column(6,
             checkboxGroupInput(
  ...
  wellPanel(
    h4(icon("star"), " Reward Parameters",
       tags$small(em(" (18_Reward_parameters.csv)"))),
    param_row("Alpha_Service_Min", "Alpha (comfort weight)", cfg_num("Alpha_Service_Min_min"), cfg_num("Alpha_Service_Min_max"), cfg_num("Alpha_Service_Min_step"), 0.1)
  ),
  ...
  wellPanel(
    h4(icon("calendar"), " Debug & Config Parameters",
       tags$small(em(" (30_Debug_and_config.csv)"))),
    checkboxGroupInput(
      "month_subset",
  ...
  wellPanel(
    ...
    fluidRow(
      ...
      column(4,
             ...
             helpText(
               ...
               "  bash \"31_SCC_Simulation/Console_code.txt\" 2",
    ...
    fluidRow(
      column(4,
             numericInput("max_requeue",
                          "Max re-queue attempts for failed configs",
                          value = cfg_num("max_requeue"),
                          min   = 0,
                          step  = 1)
      ),
      column(4,
             checkboxInput("keep_logs",
                           "Keep Slurm .err and .out log files",
                           value = as.logical(cfg_num("keep_logs")))
      ),
      column(4,
             br(),
             helpText(
               "Max re-queue: how many times failed configurations",
               "are automatically re-submitted after all jobs finish.",
               "Keep logs: if unchecked, .err/.out files are deleted",
               "after each job array finishes."
             )
      )
    )
  ),
...
server <- function(input, output, session) {
  ...
  # -------------------------------------------------------------
  {
    ...
    build_params <- reactive({
      list(
        ...
        control_type                      = as.character(input$control_type),
        ...
        Alpha_Service_Min                     = build_param_range(input$Alpha_Service_Min_min,
                                                              input$Alpha_Service_Min_max,
                                                              input$Alpha_Service_Min_step),
  ...
  # Reads scc_settings.sh from 31_SCC_Simulation/ and updates
  ...
  # -------------------------------------------------------------
  {
    session$onFlushed(function() {
      ...
      if (file.exists(settings_file)) {
        ...
        val <- extract_val("SCC_KEEP_LOGS")
        if (!is.null(val) && nzchar(val)) {
          updateCheckboxInput(session, "keep_logs", value = as.logical(as.integer(val)))
        }
        val <- extract_val("SCC_MAX_REQUEUE")
        if (!is.null(val) && nzchar(val)) {
          updateNumericInput(session, "max_requeue", value = as.integer(val))
        }
      }
    }, once = TRUE)
  }
    ...
    observeEvent(input$btn_save, {
      ...
      tryCatch({
        result <- save_optim_params(
          ...
          keep_logs      = as.integer(input$keep_logs),
          max_requeue    = as.integer(input$max_requeue)
        )
