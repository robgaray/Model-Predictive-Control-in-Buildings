...
3.  **Automatic backup**: Before modifying anything, the script creates a
    timestamped backup of `Optim_parameters.csv` as
...
4.  **Update for re-execution**: After verification,
    ...
    `Console_code.txt` to re-execute only the failed simulations.
    If all configurations pass, `Optim_parameters.csv` remains unchanged.
...
-   Works regardless of the user's current working directory (it resolves
    the project root from the script's own location).
...
-   `Verification_code.txt` — Verification script for simulation
    results (run after all jobs complete).
