...
INPUT_FILE="02_auxiliary_SCC/Optim_parameters.csv"
R_SCRIPT="Main_SCC.R"
...
IFS=';' read -r population_size iteration_number run_number \
               control_optimization_horizon control_implementation_horizon \
               control_optimization_anticipation month_subset <<< "$line"

# Trim whitespace
population_size=$(echo "$population_size" | xargs)
iteration_number=$(echo "$iteration_number" | xargs)
run_number=$(echo "$run_number" | xargs)
control_optimization_horizon=$(echo "$control_optimization_horizon" | xargs)
control_implementation_horizon=$(echo "$control_implementation_horizon" | xargs)
control_optimization_anticipation=$(echo "$control_optimization_anticipation" | xargs)
month_subset=$(echo "$month_subset" | xargs)

# Basic validation
if [[ -z "$population_size" || -z "$iteration_number" || -z "$run_number" || \
      -z "$control_optimization_horizon" || -z "$control_implementation_horizon" || \
      -z "$control_optimization_anticipation" || -z "$month_subset" ]]; then
...
echo "Number of runs=$run_number"
echo "Control optimization horizon=$control_optimization_horizon"
echo "Control implementation horizon=$control_implementation_horizon"
echo "Control optimization anticipation=$control_optimization_anticipation"
echo "Month subset=$month_subset"

# Load R module if the cluster uses modules
# module load R/4.3.1
