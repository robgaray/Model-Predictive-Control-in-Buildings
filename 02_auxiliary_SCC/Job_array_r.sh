...
IFS=';' read -r popsize iter run opt_hor cont_hor month period control forecast <<< "$line"
...
cont_hor=$(echo "$cont_hor" | xargs)
month=$(echo "$month" | xargs)
period=$(echo "$period" | xargs)
control=$(echo "$control" | xargs)
forecast=$(echo "$forecast" | xargs)

# Basic validation
if [[ -z "$popsize" || -z "$iter" || -z "$run" || -z "$opt_hor" || -z "$cont_hor" || -z "$month" || -z "$period" || -z "$control" || -z "$forecast" ]]; then
...
echo "Optimization Horizon=%$opt_hor"
echo "Control Horizon=$cont_hor m=$month" 

# Load R if the cluster uses modules
# module load R/4.3.1

# Run your R script with positional arguments
Rscript "$R_SCRIPT" "$popsize" "$iter" "$run" "$opt_hor" "$cont_hor" "$month" "$period" "$control" "$forecast"
