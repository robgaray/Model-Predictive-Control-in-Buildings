...
# -------------------------------------------------------------
# Usage instructions
#   source(file.path("02_SCC_simulation",
#                    "data_outputs_SCC.R"))
# -------------------------------------------------------------
...

{
  ...
  # -------------------------------------------------------------
  {
    if (write_main_csv_flag) {
      write.csv(Main_df,
                file.path(output_path, paste0("Main_df_computed_", suffix, ".csv")))
    }
    if (write_main_rds_flag) {
      write_rds(Main_df,
                file.path(output_path, paste0("Main_df_computed_", suffix, ".rds")))
    }
  }
  ...
  # -------------------------------------------------------------
  {
    Sinthetized_df <- data.frame(as.data.frame(parameters$optimization),
                                 ...
                                 Alpha_confort                  = scc_run_params$Alpha_confort,
    ...
    if (write_synth_csv_flag) {
      write.csv(Sinthetized_df,
                file.path(output_path, paste0("Sinthetized_df_computed_", suffix, ".csv")))
    }
    if (write_synth_rds_flag) {
      write_rds(Sinthetized_df,
                file.path(output_path, paste0("Sinthetized_df_computed_", suffix, ".rds")))
    }
