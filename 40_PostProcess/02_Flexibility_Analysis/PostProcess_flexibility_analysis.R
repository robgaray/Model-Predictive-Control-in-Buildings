...
#   18_Reward_parameters.csv (for comfort bounds)
# -------------------------------------------------------------
# Outputs
...
#     Rscript 40_PostProcess/02_Flexibility_Analysis/PostProcess_flexibility_analysis.R
...
# 0. Paths
# ----------------------------------------------------------------
if (dir.exists("40_PostProcess/02_Flexibility_Analysis")) {
  base_dir <- "40_PostProcess/02_Flexibility_Analysis"
} else if (dir.exists("01_Input")) {
  base_dir <- "."
} else {
  stop("Cannot locate 40_PostProcess/02_Flexibility_Analysis directory. ",
       "Run this script from the repository root or from ",
       "40_PostProcess/02_Flexibility_Analysis/.")
}
...
# ----------------------------------------------------------------
reward_params_file <- file.path("30_Simulation", "02_Config", "18_Reward_parameters.csv")
if (file.exists(reward_params_file)) {
  rp_df        <- read.csv(reward_params_file, comment.char = "#", strip.white = TRUE,
                           stringsAsFactors = FALSE)
  service_t_low  <- as.numeric(rp_df$value[rp_df$parameter == "Service_T_Low"])
  service_t_high <- as.numeric(rp_df$value[rp_df$parameter == "Service_T_High"])
  if (length(service_t_low)  == 0 || is.na(service_t_low))  service_t_low  <- 21
  if (length(service_t_high) == 0 || is.na(service_t_high)) service_t_high <- 26
  cat("Comfort bounds loaded: service_t_low =", service_t_low,
      ", service_t_high =", service_t_high, "\n")
} else {
  service_t_low  <- 21
  service_t_high <- 26
  cat("18_Reward_parameters.csv not found; using defaults:",
      "service_t_low =", service_t_low, ", service_t_high =", service_t_high, "\n")
}
...
for (CONT_001 in rds_files) {
  ...
  if (!inherits(df$time, "POSIXct")) {
    ...
    parsed_time <- as.POSIXct(df$time, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
    # For any NAs (date-only values), try parsing as date (assumes 00:00:00)
    na_times <- is.na(parsed_time)
    if (any(na_times)) {
      parsed_time[na_times] <- as.POSIXct(df$time[na_times], format = "%Y-%m-%d", tz = "UTC")
    }
    df$time <- parsed_time
  ...
  for (CONT_002 in seq_along(unique_dates)) {
    ...
    draw_g1 <- local({
      ...
      df_day_       <- df_day
      day_str_      <- day_str
      service_t_low_  <- service_t_low
      service_t_high_ <- service_t_high
      function() {
        y1_lim <- c(0, 40)
        ...
        # Comfort bound lines
        abline(h = service_t_low_,  lty = 3, col = "blue", lwd = 1.5)
        abline(h = service_t_high_, lty = 3, col = "blue", lwd = 1.5)
        legend("topleft",
               bty = "n",
